...
class FetchNode(BaseNode):
    ...
    def execute(self, state):
        ...
        """
        if self.verbose:
            logger.info(f"--- 开始执行 {self.node_name} 节点 ---")
        ...
        source = input_data[0]
        logger.debug(f"源数据: {source}")

        if self.input == "json_dir" or self.input == "xml_dir":
            compressed_document = [Document(page_content=source, metadata={"source": "local_dir"})]
        elif not source.startswith("http"):
            compressed_document = [Document(page_content=remover(source), metadata={"source": "local_dir"})]
        else:
            if self.node_config is not None and self.node_config.get("endpoint") is not None:
                logger.debug(f"使用代理: {self.node_config['endpoint']}")
                ...
                loader = AsyncChromiumLoader([source], headless=self.headless)

            document = loader.load()
            compressed_document = [Document(page_content=remover(str(document[0].page_content)))]

        state.update({self.output[0]: compressed_document})
        logger.info(f"状态更新完成: {self.output[0]}")
        return state
